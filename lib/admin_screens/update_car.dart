            ...
            IconButton(onPressed: ()async{
              await showDialog(
                ...
                builder: (BuildContext context){
                  return AlertDialog(
                    title: Text('Alert!'),
                    content: Text('Are you confirm to delete this car information?'),
                    actions: [
                       TextButton(onPressed: (){
                        Navigator.pop(context);
                      }, child: Text('No')),
                      TextButton(onPressed: () async{
                        await DatabaseManager().deleteCar(widget.cars.carId);
                        Navigator.pop(context);
                        Navigator.pop(context);
                      }, child: Text('Yes')),
                    ],
                        ...
                        loading ? Center(child: CircularProgressIndicator(),):Container(
                          ...
                          child: ElevatedButton(onPressed: ()async{
                            if(formKey.currentState!.validate()){
                              ...
                              await DatabaseManager().updateCar(widget.cars.carId, brandController.text, 
                                nameController.text, int.parse(priceController.text), yearController.text, 
                                bodyController.text, engineController.text, powerController.text, safetyController.text);
                             setState(() {
                                loading = false;
                              }); 
                              Navigator.pop(context);
                            }
                          }, child: Text("Update Car Info", style: TextStyle(
